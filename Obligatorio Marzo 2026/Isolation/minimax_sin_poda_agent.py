...
class MinimaxSinPodaAgent(Agent):
    ...
    def next_action(self, obs):
        action, _ = self.minimax(obs, self.player, self.max_depth)
        return action

    def heuristic_utility(self, board: Board):
        my_moves = len(board.get_possible_actions(self.player))
        enemy_moves = len(board.get_possible_actions(3 - self.player))
        eliminated = np.count_nonzero(board.grid == 3)  # 3 representa una celda eliminada
    ...
    def minimax(self, board: Board, player: int, depth: int):
    ...
    def max_value(self, board: Board, player: int, depth: int):
        ...
        for action in actions:
            ...
            next_value, _ = self.min_value(next_board, 3 - player, depth - 1)
    ...
    def min_value(self, board: Board, player: int, depth: int):
        terminal, winner = board.is_end(player)
        if terminal:
            return self._terminal_utility(winner), None
